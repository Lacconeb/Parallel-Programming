...
main( int argc, char *argv[ ] )
{
	...
	FILE* res;
	errno_t err2 = fopen_s(&res, "p5_results2.csv", "a");
	if (err2 != 0)
	{
	...
	size_t workSize = NUM_WORK_GROUPS * sizeof(float);
	...
	cl_mem dC = clCreateBuffer( context, CL_MEM_WRITE_ONLY, workSize, NULL, &status );
	if( status != CL_SUCCESS )
	...
	*/
	...
	for (int j = 1024; j < 9000000; j= 1024 * multiplier) {

		fprintf( res, "\n%8d", j);


		for (int x = 32; x < 257; x *= 2) {
			
			size_t globalWorkSize[3] = { j, 1, 1 };
			size_t localWorkSize[3] = { x,  1, 1 };
			...
			fprintf(stderr, "%8d\t%4d\t%10.3lf GigaMultsPerSecond\n",
				j, x, (double)j / (time1 - time0) / 1000000000.);
			
			fprintf(res, ",%10.3lf", (double)j / (time1 - time0) / 1000000000.);
	...
	//fprintf(res, ",%10.3lf", (double)NUM_ELEMENTS / (time1 - time0) / 1000000000.);
